...
from backend.app.bootstrap import UnsafeAuthConfigurationError, bootstrap_application, validate_auth_runtime_guardrails
...
def test_local_runtime_allows_default_auth_configuration() -> None:
    settings = Settings(runtime_environment='local')
...
def test_default_admin_password_is_rejected_in_non_local_runtime() -> None:
    settings = Settings(
    ...
    with pytest.raises(UnsafeAuthConfigurationError, match='admin_login_password'):
        validate_auth_runtime_guardrails(settings)


def test_default_hr_password_is_rejected_in_non_local_runtime() -> None:
    settings = Settings(
        runtime_environment='production',
        auth_secret_key='safe-signing-secret',
...
def test_default_secret_is_rejected_in_non_local_runtime() -> None:
    settings = Settings(
        runtime_environment='production',
