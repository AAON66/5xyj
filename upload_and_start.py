...
print("=== Uploading fixed config.py ===")
sftp = ssh.open_sftp()
sftp.put("backend/app/core/config.py", "/opt/execl_mix/backend/app/core/config.py")
sftp.close()

print("\n=== Killing old process ===")
stdin, stdout, stderr = ssh.exec_command("pkill -f 'uvicorn backend.app.main'")
...
if result and ("healthy" in result or "{" in result):
    print(f"\n[SUCCESS] 部署成功!")
