import { CloudUploadOutlined, DeleteOutlined, ExportOutlined, PlayCircleOutlined, SwapOutlined } from "@ant-design/icons";
import {
  Alert,
  Button,
  Card,
...
const PREVIEW_FIELD_LIMIT = 10;
const VIEWER_IDENTITY_FIELDS = [
  "person_name",
  "employee_id",
  "id_number",
  "company_name",
...
function buildCompareProgressState(
  steps: CompareProgressStep[],
  completedKeys: string[],
  currentKey: string | null,
...
function buildViewerFields(
  data: BatchCompareResult,
  rows: BatchCompareResult["rows"],
  showAllFields: boolean,
): string[] {
  const identityFields: string[] = VIEWER_IDENTITY_FIELDS.filter((field) => data.fields.includes(field));
  if (showAllFields) {
    return [...identityFields, ...data.fields.filter((field) => !identityFields.includes(field))];
  }

  const highlightedFields = new Set<string>();
  for (const row of rows) {
    row.different_fields.forEach((field) => {
  ...
  if (!highlightedFields.size) {
    for (const field of data.fields) {
      if (identityFields.includes(field)) {
        continue;
      }
      const hasData = rows.some((row) => hasValue(row.left.values[field] ?? null) || hasValue(row.right.values[field] ?? null));
      if (hasData) {
        highlightedFields.add(field);
      }
      if (highlightedFields.size >= PREVIEW_FIELD_LIMIT) {
        break;
      }
    }
  }

  const orderedFields = data.fields.filter((field) => highlightedFields.has(field) && !identityFields.includes(field));
  return [...identityFields, ...orderedFields];
}
...
function cloneRowWithFieldUpdate(
  ...
  const rows = data.rows.map((row) => {
    ...
    const differentFields = data.fields.filter(
      (candidateField) => normalizeCellValue(left.values[candidateField] ?? null) !== normalizeCellValue(right.values[candidateField] ?? null),
    );
    const leftExists = Object.values(left.values).some((value) => normalizeCellValue(value) !== null) || Boolean(left.source_file_name);
    const rightExists = Object.values(right.values).some((value) => normalizeCellValue(value) !== null) || Boolean(right.source_file_name);
    let diffStatus = "same";
    if (!leftExists && rightExists) {
      diffStatus = "right_only";
    } else if (leftExists && !rightExists) {
...
export function ComparePage() {
  ...
  const viewerFields = useMemo(
    () => (displayCompareData ? buildViewerFields(displayCompareData, pagedRows, showAllFields) : []),
    [displayCompareData, pagedRows, showAllFields],
  );
  ...
  async function resolveSideBatchId(
    side: Side,
    progressHandlers?: { activateStep: (key: string) => void; completeStep: (key: string) => void },
  ): Promise<string> {
  ...
  async function handleRunCompare() {
    ...
    try {
      ...
      if (resolvedLeftBatchId === resolvedRightBatchId) {
        throw new ApiClientError("左侧和右侧不能使用同一个批次。");
      }
        ...
        {mode === "existing" ? (
          <div>
            ...
            <Select
              ...
              placeholder="请选择批次"
              onChange={(value) => (side === "left" ? setLeftBatchId(value) : setRightBatchId(value))}
              loading={loadingBatches}
              options={batches.map((batch) => ({ value: batch.id, label: batchLabel(batch) }))}
            />
          </div>
        ) : (
          <>
            ...
            {localEntries.length > 0 ? (
                ...
                {localEntries.map((entry) => (
                  <div
                    ...
                    style={{
                      ...
                      gap: 8,
                      padding: "4px 8px",
                      background: colors.FILL_QUATERNARY,
                      borderRadius: 4,
                    }}
                  >
                    <div>
                    ...
                    <Button type="link" danger size="small" onClick={() => removeLocalFile(side, entry.id)}>
                      删除
                    </Button>
                  </div>
                ))}
              ...
              <Text type="secondary" style={{ fontSize: 13 }}>
                上传 Excel 后，这一侧会按本地文件生成临时批次参与对比。
              </Text>
            )}
          </>
        )}
    ...
    <div>
      ...
      <Card title="对比表格类型" style={{ marginBottom: 16 }}>
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} lg="auto">
            ...
            <Select
              data-testid="compare-table-type"
              value={compareTableType}
              onChange={(value) => setCompareTableType(value)}
              options={[
      ...
      {runningCompare && compareProgress ? (
        <Card style={{ marginBottom: 16 }}>
          ...
          <div style={isCompact ? { overflowX: "auto", paddingBottom: 4 } : undefined}>
            <Steps
              size="small"
              responsive={false}
              current={compareProgress.currentStep - 1}
              style={{ marginTop: 12, minWidth: isCompact ? 720 : undefined }}
              items={compareProgress.steps.map((step) => ({
                title: step.label,
                description: compareProgress.completedKeys.includes(step.key)
                  ? "已完成"
                  : compareProgress.currentKey === step.key
                    ? "进行中"
                    : "等待中",
              }))}
            />
        ...
        <Row gutter={[12, 12]} align="middle" wrap>
          <Col xs={24} lg="auto" flex="auto">
            <Input.Search
              ...
              style={{ maxWidth: isCompact ? "100%" : 420 }}
              allowClear
            />
          </Col>
          <Col>
          ...
          <Col>
            <Checkbox checked={showAllFields} onChange={(event) => setShowAllFields(event.target.checked)}>
              展开全部字段
            </Checkbox>
          </Col>
          <Col>
            <Select
              ...
              options={PAGE_SIZE_OPTIONS.map((value) => ({ value, label: `${value} 行/页` }))}
              style={{ width: 100 }}
            />
          </Col>
        </Row>
        ...
        {displayCompareData ? (
          <Row gutter={[16, 16]} style={{ marginTop: 12 }}>
            <Col xs={24} sm={12} md="auto">
              <Statistic title="总对比行数" value={displayCompareData.total_row_count} />
            </Col>
            <Col xs={24} sm={12} md="auto">
              <Statistic title="存在差异" value={displayCompareData.changed_row_count} valueStyle={{ color: colors.WARNING }} />
            </Col>
            <Col xs={24} sm={12} md="auto">
              <Statistic title="仅左侧" value={displayCompareData.left_only_count} valueStyle={{ color: colors.BRAND }} />
            </Col>
            <Col xs={24} sm={12} md="auto">
              <Statistic title="仅右侧" value={displayCompareData.right_only_count} valueStyle={{ color: colors.WARNING }} />
            </Col>
            <Col xs={24} sm={12} md="auto">
              <Statistic title="完全一致" value={displayCompareData.same_row_count} />
            </Col>
          </Row>
        ) : null}
      </Card>
      ...
      {!displayCompareData ? (
        ...
        <Card>
          ...
          >
            <div>
              <Text strong>Workbook Diff</Text>
              <Text type="secondary" style={{ marginLeft: 8 }}>
                第 {currentPage} / {totalPages} 页，当前窗口 {pagedRows.length} 行，展示 {viewerFields.length} 列
              </Text>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <Button size="small" disabled={currentPage <= 1} onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}>
                上一页
              </Button>
              <Button
                size="small"
          ...
          <CompareWorkbookDiff
            fields={viewerFields}
            rows={pagedRows}
            leftLabel={`左侧 · ${displayCompareData.left_batch.batch_name}`}
            rightLabel={`右侧 · ${displayCompareData.right_batch.batch_name}`}
            editable
            emptyDescription="当前窗口没有可展示的对比结果。"
